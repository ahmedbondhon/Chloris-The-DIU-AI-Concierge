"""
Run once to populate the database with a realistic demo student.
Command: python seed_student_data.py
"""
import sys, os
...
from models.db_models import (
    Base, User, StudentProfile, Course,
...
def seed():
    ...
    if not profile:
        ...
        db.commit()
        db.refresh(profile)
        print(f"  Profile created: ID {profile.student_id}, "
              f"CGPA {profile.cgpa}")
    ...
    attendance_data = [
        ("CSE301", 20, 17),   # 85% — good
        ("CSE315", 18, 16),   # 88% — good
        ("CSE325", 20, 14),   # 70% — warning
        ("CSE341", 16, 15),   # 93% — excellent
        ("MAT201", 18, 10),   # 55% — critical
        ("ENG201", 14, 13),   # 92% — excellent
    ]
    ...
    db.commit()
    print(f"  Attendance: {len(attendance_data)} course records created")
